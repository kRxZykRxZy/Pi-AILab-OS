...
# Respect GGUF's explicit RoPE dimension and call the modular implementation.
...
rope_new = "const size_t rd=arch_.rope_dim?arch_.rope_dim:hd;for(size_t h=0;h<arch_.heads;h++)piai::inference::apply_rope(q.data()+h*hd,rd,pos,arch_.rope_theta);for(size_t h=0;h<arch_.kv_heads;h++)piai::inference::apply_rope(kk.data()+h*hd,rd,pos,arch_.rope_theta);"
text = text.replace(rope_old, rope_new)

# Remove the old local RoPE implementation now that it is independently compiled.
text = remove_function(text, "static void rope(")
...
# Keep the exact FP16 lookup-table optimization for scalar/quantized element reads.
hf_old = 'static float hf(uint16_t h){uint32_t s=h>>15,e=(h>>10)&31,f=h&1023;if(!e)return(s?-1.f:1.f)*std::ldexp((float)f,-24);if(e==31)return f?NAN:(s?-INFINITY:INFINITY);return(s?-1.f:1.f)*std::ldexp(1.f+f/1024.f,(int)e-15);}'
