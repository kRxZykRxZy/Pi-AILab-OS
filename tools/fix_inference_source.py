...
old = "for(size_t h=0;h<arch_.heads;h++)rope(q.data()+h*hd,hd,pos,arch_.rope_theta);for(size_t h=0;h<arch_.kv_heads;h++)rope(kk.data()+h*hd,pos,arch_.rope_theta);"
# Keep compatibility with the exact current source spelling as well.
old2 = "for(size_t h=0;h<arch_.heads;h++)rope(q.data()+h*hd,hd,pos,arch_.rope_theta);for(size_t h=0;h<arch_.kv_heads;h++)rope(kk.data()+h*hd,hd,pos,arch_.rope_theta);"
...
text = text.replace(old, new).replace(old2, new)
...
# Shared architecture-specific AXPY kernel for attention values. This is scalar
# on ARMv6 and NEON-vectorized on ARMv7/AArch64.
old = "for(size_t j=0;j<hd;j++){float u=0;for(size_t pp=0;pp<=pos;pp++)u+=sc[pp]*cache_.value(l,pp,kh)[j];a[h*hd+j]=u;}"
new = "std::fill(a.begin()+h*hd,a.begin()+(h+1)*hd,0.f);for(size_t pp=0;pp<=pos;pp++)piai::compute::axpy_f32(a.data()+h*hd,cache_.value(l,pp,kh),sc[pp],hd);"
text = text.replace(old, new)
