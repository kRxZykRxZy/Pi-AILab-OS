...
void init(){
 R(UART+0x30)=0;
 uint32_t f=R(GPIO+0x04); f&=~(7u<<12); f&=~(7u<<15); f|=(4u<<12)|(4u<<15); R(GPIO+0x04)=f;
 R(UART+0x44)=0; R(UART+0x24)=0; R(UART+0x28)=0; R(UART+0x24)=26; R(UART+0x28)=3;
 R(UART+0x2c)=(3u<<5); R(UART+0x30)=(1u<<9)|(1u<<8)|1u;
}
