...
static bool value(const uint8_t*&p,const uint8_t*e,Type t,Value&v){v.type=t;switch(t){case Type::UINT8:case Type::INT8:case Type::BOOL:return rd(p,e,nullptr,1);case Type::UINT16:case Type::INT16:return rd(p,e,nullptr,2);case Type::UINT32:case Type::INT32:case Type::FLOAT32:return rd(p,e,nullptr,4);case Type::UINT64:case Type::INT64:case Type::FLOAT64:return rd(p,e,nullptr,8);case Type::STRING:return text(p,e,v.string);case Type::ARRAY:{uint32_t at;uint64_t n;if(!r(p,e,at)||at>12||!r(p,e,n)||n>(1ull<<24))return false;v.array.resize((size_t)n);for(auto&x:v.array)if(!value(p,e,(Type)at,x))return false;return true;}default:return false;}}
Model::~Model(){close();}
