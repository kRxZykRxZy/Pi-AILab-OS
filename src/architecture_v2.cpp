...
bool detect_architecture(const gguf::Model&m,Architecture&a){const auto*g=m.metadata("general.architecture");if(!g||!g->is_string())return false;a=Architecture{};a.name=g->string;const std::string p=a.name+".";if(!g32(m,p+"block_count",a.layers)||!g32(m,p+"embedding_length",a.hidden)||!g32(m,p+"attention.head_count",a.heads)||!g32(m,p+"feed_forward_length",a.intermediate))return false;if(!g32(m,p+"attention.head_count_kv",a.kv_heads))a.kv_heads=a.heads;if(!g32(m,p+"context_length",a.context))a.context=2048;if(!g32(m,p+"rope.dimension_count",a.rope_dim))a.rope_dim=a.hidden/a.heads;const auto*t=m.metadata("tokenizer.ggml.tokens");if(!t||!t->is_array())return false;a.vocab=t->array.size();float x;if(f32(m.metadata(p+"attention.layer_norm_rms_epsilon"),x))a.eps=x;if(f32(m.metadata(p+"rope.freq_base"),x))a.rope_theta=x;return a.layers&&a.hidden&&a.heads&&a.kv_heads&&a.vocab&&a.intermediate&&a.hidden%a.heads==0&&a.heads%a.kv_heads==0&&a.rope_dim&&a.rope_dim<=a.hidden/a.heads&&a.rope_dim%2==0;}
}
