...
void q5_1_dot(const float*x,const void*ww,size_t n,float&o){const uint8_t*p=(const uint8_t*)ww;float a=0.f;for(size_t b=0,i=0;i<n;i+=32,b+=24){uint16_t ds,ms;std::memcpy(&ds,p+b,2);std::memcpy(&ms,p+b+2,2);uint32_t qh;std::memcpy(&qh,p+b+4,4);float d=h(ds),m=h(ms);for(size_t j=0;j<32&&i+j<n;j++){uint8_t z=p[b+8+j/2];int q=(j&1)?z>>4:z&15;q|=((qh>>j)&1)<<4;a+=x[i+j]*(d*q+m);}}o=a;}
}
