...
class Engine {Architecture arch_;Vocabulary vocab_;KVCache cache_;Sampling sampling_;std::mt19937_64 rng_;const gguf::Model*model_=nullptr;TensorBinding embedding_{},output_{},norm_final_{};std::vector<TensorBinding>q_,k_,v_,o_,norm1_,norm2_,ffn1_,ffn2_,ffn3_;public:bool load(const gguf::Model&);bool generate(const std::string&,size_t,std::vector<std::string>&,const std::function<bool(const std::string&)>& on_token={});const Architecture&architecture()const{return arch_;}const TensorBinding&embedding()const{return embedding_;}};
}
