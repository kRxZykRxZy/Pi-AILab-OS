...
static constexpr uintptr_t INTERRUPT=EMMC+0x30, IRPT_EN=EMMC+0x38, IRPT_MASK=EMMC+0x34;
static constexpr uintptr_t BLKSIZECNT=EMMC+0x04;
...
bool read(uint64_t lba,uint32_t count,void* buffer){
 if(!device.ready||!buffer||!count||lba>0xffffffffull)return false;
 uint8_t* out=(uint8_t*)buffer;
 r(BLKSIZECNT)=((count&0xffff)<<16)|512;
 uint32_t arg=(uint32_t)lba;
 ...
 r(INTERRUPT)=0xffff; return true;
}
